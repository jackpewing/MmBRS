...
dpath = 'G:\Shared drives\SWAL_Arctic\Research_projects\JackBRS\Arctic_shiptxClicks\output\Stage_two\Combine_all';
load(fullfile(dpath, 'mmdata_20240128_daily_not4analysis.mat'));

% Convert the table to a timetable
tt = table2timetable(mmdata);

% Select required columns, including the new narwhal presence column
tt = tt(:, {'MmPres_eff_adj', 'Ice_pc'});

% Convert the timetable to daily
tt_daily = retime(tt, 'daily', 'sum');
tt_daily.Ice_pc = retime(tt, 'daily', 'mean').Ice_pc;

% Define the start and end of the period of interest
start_date = datetime(2016, 1, 1);
end_date = datetime(2021, 12, 31);

% Generate a list of all hourly timestamps from start_date to end_date
all_hours = (start_date:hours(1):end_date)';

% Find the missing hours by comparing with tt
missing_hours = setdiff(all_hours, tt.Time);

% Identify contiguous blocks of missing hours
diff_missing = [hours(2); diff(missing_hours)];
breaks = find(diff_missing > hours(1));

% Determine start and end of these blocks
missing_starts = missing_hours(breaks);
missing_ends = [missing_hours(breaks(2:end)-1); missing_hours(end)];

unique_years = unique(year(tt_daily.Time));
...
for i = 1:length(unique_years)
    year_data = tt_daily(year(tt_daily.Time) == unique_years(i), :);
    ...
    title([num2str(unique_years(i))], 'FontSize', 12);
    grid on;
